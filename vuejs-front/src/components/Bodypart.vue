<template>
  <div class="db-box" v-if="bodypart">
    ...
    <div class="row">
      <div class="col-md-8 col-sm-12 col-xs-12">
        ...
        <div class="box collapsed-box">
          <app-box-header :title="'Tags ('+(bodypart.tags?bodypart.tags.length:0)+')'" collapsable="true" collapsed="true" icon="fa-tags"></app-box-header>
          <div class="box-body">
            <span class="label label-primary margin-right" v-for="tag in bodypart.tags" :key="tag.internalid"><i class="fa fa-tag"></i> {{ tag.tag }}</span>
            ...
            <transition name="fade">
              <div v-if="addTag">
                ...
                <div class="row">
                  <div class="col-md-8">
                    <auto-suggest placeholder="New tag" :options="bodypartsTagsCodeList" v-model="newTag"></auto-suggest>
                  </div>
                  <div class="col-md-4">
      ...
      <div class="col-md-4 col-sm-12 col-xs-12">
        <div class="box">
          <app-box-header title="Photo" collapsable="true" icon="fa-photo" 
              :editable="canedit" :editlink="'/bodypart/'+bodypart.internalid+'/edit/image/'+current"
              :otherable="canedit" :otherlink="'/bodypart/'+bodypart.internalid+'/edit/image/'+(bodypart.nbpictures*1+1)" othericon="fa-plus-square-o" othername="Add photo"></app-box-header>
          <div class="box-body db-image">
            <img :src="resources.img_url+'/images/bodyparts/'+bodypart.internalid+'/'+current+'/thumb'"  v-if="bodypart.nbpictures > 0"/>
            <img :src="resources.img_url+'/images/unknown'" v-else />
            <div v-if="bodypart.nbpictures > 1" style="text-align:center;">
              <i class="fa fa-chevron-left" style="float:left;" @click="prevPhoto()"></i>
              <span style="text-align:center; font-weight:bold;">{{ bodypart.nbpictures }} photos available</span>
    ...
    <div class="row">
      <div class="col-md-12 col-sm-12 col-xs-12" v-if="photo4bodypart.length > 0">
        <div class="box collapsed-box">
          <app-box-header :title="'Photos ('+photo4bodypart.length+')'" collapsable="true" collapsed="true" icon="fa-photo"></app-box-header>
          <div class="box-body">
  ...
  <div class="row" v-else>
    <div class="col-md-12" v-if="bodyparts.length === 0">
...
export default {
  ...
  computed: {
    ...Vuex.mapGetters(['boxes', 'nendoroids', 'bodyparts', 'photos', 'photobodyparts',
      'authenticated', 'canedit', 'bodypartsTagsCodeList']),
    bodypart () {
  ...
  methods: {
    ...Vuex.mapActions(['collectBodypart', 'uncollectBodypart',
      'validateBodypart', 'unvalidateBodypart',
      'favoriteBodypart', 'unfavoriteBodypart',
      'tagBodypart',
      'retrieveSingleBodypart', 'retrieveBoxesForBodypart', 'retrieveNendoroidsForBodypart']),
    ...
    nextPhoto () {
      this.current = (this.current === (this.bodypart.nbpictures * 1)) ? 1 : this.current + 1
    },
    prevPhoto () {
      this.current = (this.current === 1) ? this.bodypart.nbpictures : this.current - 1
    }
  },
  mounted () {
    if (this.bodyparts.length === 0) {
      this.retrieveSingleBodypart({
        'context': this,
        'bodypartid': this.$route.params.id
      })
    }
    if (this.boxes.length === 0) {
      this.retrieveBoxesForBodypart({
        'context': this,
        'bodypartid': this.$route.params.id
      })
    }
    if (this.nendoroids.length === 0) {
      this.retrieveNendoroidsForBodypart({
        'context': this,
        'bodypartid': this.$route.params.id
      })
    }
