...
function count_nendoroidBodyParts($nendoroid_id)
{
  ...
  $req = $bdd->prepare("SELECT count(*) AS count
                        FROM body_parts AS bp
                        WHERE bp.nendoroid_id = :nendoroid_id");
...
function get_singleBodypart($bodypart_id)
{
  ...
  $req = $bdd->prepare("SELECT bp.internalid,
                        bp.box_id, b.name AS box_name, b.type AS box_type,
                        ...
                        LEFT JOIN nendoroids AS n ON bp.nendoroid_id = n.internalid
                        LEFT JOIN users AS uc ON bp.creator = uc.internalid
                        LEFT JOIN users AS ue ON bp.editor = ue.internalid
                        WHERE bp.internalid = :bodypart_id");
...
function add_singleBodypart($box_id,$nendoroid_id,$bodypart_main_color,$bodypart_main_color_hex,$bodypart_second_color,$bodypart_second_color_hex,$bodypart_part,$bodypart_description,$userid)
{
  ...
  $req = $bdd->prepare("INSERT INTO body_parts(box_id,nendoroid_id,main_color,main_color_hex,second_color,second_color_hex,part,description,creator,creation,editor,edition)
                        VALUES(:box_id,:nendoroid_id,:main_color,:main_color_hex,:second_color,:second_color_hex,:part,:description,:creator,NOW(),:editor,NOW())");
  $req->bindParam(':box_id',$box_id);
  $req->bindParam(':nendoroid_id',$nendoroid_id);
