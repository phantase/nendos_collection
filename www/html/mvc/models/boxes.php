...
function get_singleBox($boxinternalid)
{
  ...
  $req = $bdd->prepare("SELECT b.internalid, b.name, b.type, b.creator, uc.username AS creator_name, b.creation, b.editor, ue.username AS editor_name, b.edition, NOW() AS now FROM boxes AS b LEFT JOIN users AS uc ON b.creator = uc.internalid LEFT JOIN users AS ue ON b.creator = ue.internalid WHERE b.internalid = :boxinternalid");
...
function add_singleBox($boxname,$boxtype,$userid)
{
  ...
  $req = $bdd->prepare("INSERT INTO boxes(name,type,creator,creation,editor,edition) VALUES(:boxname,:boxtype,:creator,NOW(),:editor,NOW())");
